...
為尋求在 Recall 和 Precision 之間的最佳平衡，引入了閾值微調。
...
- **備註：關於 GPU 預測性能的警告**：在最終的模型版本中，執行預測時可能會出現一個關於 `mismatched devices` 的 `UserWarning`。這是因為模型在 GPU 上運行，而輸入的稀疏矩陣數據在 CPU 上。曾嘗試透過更新 `xgboost` 版本並使用 `xgb.DMatrix(..., device="cuda")` 的語法來修正，但因 API 不相容而導致 `TypeError`。另一個 `cupy` 方案則有引發記憶體錯誤的風險。因此，最終決定接受這個不影響結果正確性的性能警告，以確保程式碼的穩定性和通用性。
...
2.  **執行主程式**：
    *   在專案根目錄下，執行 `main.py`：
    ...
    python main.py
    ```
    *   程式會顯示一個互動式選單，可根據提示選擇要運行的模型 (1: XGBoost, 2: CNN, 3: Transformer)。
...
2.  **啟動容器並執行程式**：
    *   執行以下指令來啟動容器。此指令會將本機的專案資料夾掛載到容器內，方便同步程式碼和存取輸出檔案。
    ```bash
    docker run --gpus all -it --rm -v "%cd%:/app" protein-ptm-predictor
    ```
    *   **在容器內**：成功啟動後，終端機提示符會改變。此時，已在 Docker 環境內，可透過 `python main.py` 來啟動選單，或直接執行 `scripts/` 下的任一腳本。

### 3.3. 退出容器 (Exiting the Container)

當在容器的終端機中完成操作後，只需輸入以下指令並按下 Enter，即可退出並自動關閉/刪除容器：
```bash
exit
```
...
可直接修改此檔案中的數值，來調整訓練的週期 (`epochs`)、批次大小 (`batch_size`)，以及 XGBoost 超參數搜索的並行數 (`n_jobs`)，而無需更動任何 Python 程式碼。這對於在不同硬體上進行實驗或微調模型非常方便。

## 4. 未來工作 (Future Work)

### 4.1. (已完成) 進階生物特徵工程
此階段任務已成功完成，透過為模型引入生物學領域知識，極大地提升了性能。
- **策略**：除了 k-mer 頻率，計算並引入了三組具有明確生物學意義的特徵。
...
*   **XGBoost 模型 (基於 Tabular/Engineered Features):** **(優化完畢)**
    *   **系統性的超參數搜索 (Systematic Hyperparameter Search):** **(已完成)** 使用 `RandomizedSearchCV` 進行了自動化搜索。結果表明，手動設定的參數已接近最佳，此步驟並未帶來顯著的性能提升，但驗證了模型的穩定性。
...
*   **CNN 模型 (基於 Sequence - 自動學習局部模式):**
    *   **架構對比實驗 (已完成):**
        1.  **簡單 1D-CNN + 樣本加權**: 實現了一個基礎但高效的 1D-CNN 模型，在加入樣本加權處理類別不平衡後，取得了 **0.729979625390496** 的平均 ROC AUC (使用 batch size 32)。
        2.  **1D-ResNet + 自動超參數搜索**: 為了探索更複雜的架構，將模型升級為 1D-ResNet，並使用 `KerasTuner` 進行了自動化的超參數搜索。
        3.  **1D-CNN-LSTM (實驗結果：平均 ROC AUC ≈ 0.7340825531491221)**: 在基礎 1D-CNN 上引入 LSTM 層，嘗試捕捉序列的長距離依賴關係。(使用 batch size 32)
    *   **實驗結論**: 出乎意料地，更複雜的 ResNet 架構在經過 `KerasTuner` 的搜索後，其最佳性能 (平均 ROC AUC ≈ 0.7440916158813945 (使用 batch size 32)) **反而不如**結構更簡單的 1D-CNN 模型。這可能是由於數據集規模、搜索空間限制等原因。
    *   **最終策略**: 保留了兩個版本的腳本 (`scripts/CNN_MultiLabel.py` 和 `scripts/CNN_ResNet.py`)，並在 `main.py` 選單中提供選項，將表現更優的簡單 1D-CNN 標註為「較佳」。
...
在本專案中，使用了 XGBoost、CNN 和 Transformer 三種不同的模型。它們在「可解釋性」上存在明顯的層次差異。

### 5.1. XGBoost (可解釋性：高)

*   **為何可解釋？**
    *   **特徵重要性 (Feature Importance)**：XGBoost 能直接計算並排列出所有手動設計的特徵（如 AAC, DPC, k-mers 等）對預測的貢獻度。這能直觀地理解模型是基於哪些「規則」來做判斷的。
*   **如何解釋？**
    *   透過分析排名前列的特徵，可得出宏觀結論，例如「DPC 特徵普遍比 AAC 重要」，或「某個特定理化性質是關鍵驅動因素」。
...
*   **如何「窺探」黑盒子？**
    *   **注意力圖 (Attention Map)**：這是 Transformer 獨有的可解釋性工具。「自註意力機制」所計算的胺基酸之間的「關注分數」可被視覺化成熱力圖。這有助於直觀理解模型學到的**長距離和短距離依賴關係**，為 CNN 和 XGBoost 所無法直接提供的洞見。

### 總結

| 模型 | 可解釋性 | 主要方法 | 能告訴我們什麼？ |
| :--- | :--- | :--- | :--- |
| **XGBoost** | **高** | 特徵重要性 (Feature Importance) | 「哪些**手動設計的特徵**最重要？」 |
...
本節探討如何將遷移學習 (Transfer Learning) 與現有模型進行結合，並透過模型堆疊 (Stacking) 實現最終的性能突破。
...
**結果**： 透過 Stacking 結合這些模型，可同時整合「生化學家的嚴謹」與「AI 的直覺」。XGBoost 能夠修正 Transformer 偶爾發生的低級統計錯誤；Transformer 則可彌補 XGBoost 在結構理解上的不足。

### 6.2. 關於「遷移學習 (Embedding)」的策略抉擇
在 Stacking 中可引入透過大型蛋白質語言模型（如 ESM-2）預先計算好的 Embedding，這提供了兩種不同的策略：

*   **策略 A：追求極致的「多樣性 (Diversity)」 (推薦)**
    *   **做法**： 保留現有的 「AUC 0.746 的手工特徵 XGBoost」，與 Transformer 進行 Stacking。
...
*   **策略 B：追求單體模型的「強大」**
    ...
    *   **風險**： 儘管 XGBoost 性能增強，但其與 Transformer 的相似性可能增加。由於兩者皆讀取 Embedding，錯誤模式或將相似。兩個強大但相似的模型，其組合後的性能提升可能不如預期（邊際效應遞減）。

**結論**：並無衝突。保留此「手工特徵的 XGBoost」模型極具前瞻性。不應因其 AUC 僅為 0.746 而將其捨棄。在未來的 Stacking 階段，其提供的「物理視角」是 Transformer 難以完全取代的獨特資產。
...
graph LR
    ...
    Input --> EmbedModel[ESM-2 / Transformer]

    subgraph "Level 1: Base Models (專家群)"
        FeatureEng --> XGB_Manual["XGBoost (手工特徵)"]
        EmbedModel --> Transformer["Transformer"]
...
這個架構的優勢包括：
*   **XGBoost (手工)**： 維持了生化物理性質的底線（解釋性強）。
*   **Transformer**： 提供了強大的結構與語意理解。
*   **XGBoost (Embeddings)**： 作為額外專家，提供了另一種視角。
*   **Stacking (Level 2)**： 負責分配權重。例如，對於短序列，可能發現 XGBoost 的預測較為精確；對於長而複雜的序列，則會更側重 Transformer 的結果。

### 6.4. 堆疊策略與風險管理 (Stacking Strategy & Risk Management)

在使用包含多個以 Embedding 為基礎的模型（如 Transformer 和以 Embedding 為輸入的 XGBoost）進行堆疊時，需要注意**「同溫層效應」或「資訊重疊」**的風險。

*   **風險分析**：
    *   架構圖中的 Transformer 模型 (產生預測 P2) 和 XGBoost (Embeddings) 模型 (產生預測 P3) 都以大型語言模型 (如 ESM-2) 產生的 Embedding 作為輸入源。
    *   如果 ESM-2 的 Embedding 在某類樣本上本身就存在偏差或錯誤，那麼 P2 和 P3 很可能會「同時犯錯」。
    *   在第二層的 Meta Learner 中，兩個錯誤但相似的預測，可能會以多數票的形式，壓過來自完全不同視角（手工特徵）的 P1 的正確意見，導致最終決策失敗。

*   **風險緩解**：
    *   雖然 P2 和 P3 的源頭相同，但它們的「思考方式」不同（端到端的 Transformer vs. 基於固定特徵尋找分割點的 XGBoost），這能在一定程度上降低風險。

*   **行動計畫 (Action Plan)**：
    *   **相關性檢查 (Correlation Check)**：在訓練完第一層的所有基礎模型後，計算 P1, P2, P3 在驗證集上的預測機率之間的皮爾森相關係數 (Pearson Correlation)。如果 P2 和 P3 的相關性過高（例如 > 0.95），則證明 P3 提供的「新資訊」非常有限，可能是冗餘的。
    *   **使用帶有 L1 正規化的 Meta Learner**：在第二層，不使用簡單的線性回歸，而是使用 **Lasso Regression (帶有 L1 懲罰項的邏輯回歸)**。Lasso 的特性是，當它發現兩個輸入特徵高度相關時，它會傾向於將其中一個的權重強制縮減為零。這相當於讓演算法自動判斷：「P3 模型是否多餘？以及是否應在最終決策中考慮它？」

**最終建議**：可採用完整的三模型堆疊架構，但務必在第二層（Meta Learner）使用 Lasso 等具備正規化（特徵選擇）能力的模型。如此，即可透過數據驅動而非僅憑先驗假設，決定最終模型的最佳組合，從而避免人為設計導致的偏見。