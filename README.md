...
2.  **啟動容器並執行程式**：
    *   執行以下指令來啟動容器。此指令會將本機的專案資料夾掛載到容器內，方便同步程式碼和存取輸出檔案。
    ...
    ```
    *   **在容器內**：成功啟動後，終端機提示符會改變。此時，已在 Docker 環境內，可以透過 `python main.py` 來啟動選單，或直接執行 `scripts/` 下的任一腳本。
...
*   **CNN 模型 (基於 Sequence - 自動學習局部模式):**
    *   **架構對比實驗 (已完成):**
        ...
        3.  **1D-CNN-LSTM (實驗中)**: 在基礎 1D-CNN 上引入 LSTM 層，嘗試捕捉序列的長距離依賴關係。
...
*   **如何解釋？**
    *   透過分析排名前列的特徵，可以得出宏觀結論，例如「DPC 特徵普遍比 AAC 重要」，或「某個特定理化性質是關鍵驅動因素」。
...
**結論**：沒有衝突，保留這個「手工特徵的 XGBoost」是非常有遠見的。千萬不要因為它的 AUC 只有 0.746 就把它丟掉。在未來的 Stacking 階段，它提供的「物理視角」是 Transformer 很難完全取代的獨特資產。
...
*   **行動計畫 (Action Plan)**：
    ...
    *   **使用帶有 L1 正規化的 Meta Learner**：在第二層，不使用簡單的線性回歸，而是使用 **Lasso Regression (帶有 L1 懲罰項的邏輯回歸)**。Lasso 的特性是，當它發現兩個輸入特徵高度相關時，它會傾向於將其中一個的權重強制縮減為零。這相當於讓演算法自動幫我們判斷：「P3 模型是否多餘？我是否應該在最終決策中考慮它？」

**最終建議**：可以大膽地使用完整的三模型堆疊架構，但務必在第二層（Meta Learner）使用 Lasso 等具備正規化（特徵選擇）能力的模型。這樣，我們就能以數據驅動的方式，而非僅靠先驗假設，來決定最終模型的最佳組合，從而避免人為設計導致的偏見。