...
task_name = 'classify_multilabel_cnn_lstm'
...
# --- 6. 模型建構 (1D-CNN-LSTM) ---
def build_cnn_lstm_model(input_shape, num_targets):
    model = keras.Sequential([
        ...
        layers.Conv1D(filters=128, kernel_size=5, activation='relu', padding='same'),
        layers.BatchNormalization(),
        layers.MaxPooling1D(pool_size=2), # 池化層以減少序列長度
        layers.LSTM(64), # LSTM 層來處理序列資訊
...
cnn_model = build_cnn_lstm_model(input_shape, len(TARGET_COLS))
cnn_model.summary()
...
submission_filename = f"cnn_lstm_submission_{timestamp}.csv"
...
scoreboard_file = os.path.join(project_root, "run", "scoreboard_cnn_lstm.csv")
